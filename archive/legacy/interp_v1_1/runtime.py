...
def seed(purpose, key):
    return int.from_bytes(hashlib.sha256(f'20260909|experiment-spec-v1.1|{purpose}|{key}'.encode()).digest()[:8],'big')
...
def verify_inputs(root):
    root=Path(root); config=json.loads((root/'experiment_v1_1/configs/language.json').read_text())
    ...
    configs=json.loads((root/'experiment_v1_1/configs/config_set_manifest.json').read_text())

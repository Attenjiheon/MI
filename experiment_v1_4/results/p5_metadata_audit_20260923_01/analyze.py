...
for name,item in sorted(probes.items()):
 ...
 r=item['result'];statuses[r['status']]+=1;by_layer[name.split('_l')[1].split('_')[0]]+=1
...
summary=dict(status='passed_metadata_checks_only',archive_sha256=sha(ARCHIVE),files_verified=count,contract_sha256=ch,
 ...
 'Snapshot only: absent files may still be running or not yet mirrored. This return includes CPU session manifests; they do not establish current live status.',

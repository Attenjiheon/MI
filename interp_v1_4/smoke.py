"""v1.4 integration checks for the frozen P3 candidate. Outputs are DEBUG ONLY.

The 12-block architecture and token-only read4 objective are exercised on independent
debug fixtures: shapes and parameter count, causal/PAD masking, RoPE, gradient accumulation,
checkpoint/resume bitwise equality, and the real selection/gate arithmetic paths on a small
slice of `select/`. `gate/` and `test/` files are never scored here.
"""
...
from .behavior import evaluate_first_repeat, evaluate_records, gate, without_rows
...
from .integration import check_interpretation

CELLS = ("deepwide12_read4",)
SMOKE_CELLS = 42
SMOKE_SEQUENCES = 24
...
def check_resume(model, optimizer, state, sequences, out, cell, loss_id, inputs, device, architecture, microbatch):
    ...
    expected_log = lm_update(model, optimizer, following, state, loss_id=loss_id, microbatch=microbatch)
...
def run(root, out, device):
    ...
    for cell in CELLS:
        ...
        assert parameter_count(model) == expected_parameters, cell
        ...
        microbatch = None
        for candidate in design["training"]["smoke_microbatch_candidates_in_order"]:
            model.load_state_dict(initial)
            optimizer = lm_optimizer(model)
            state = Progress()
            try:
                # A valid maximum-length 302-token grammar fixture bounds every
                # production train/select/gate sequence, not just short debug rows.
                longest = [1] + [3, 9, 13, 3, 10, 13, 3, 11, 13, 3, 12, 13]
                longest += ([3, 9, 13] * 3 + [8, 9, 13]) * 24 + [2]
                assert len(longest) == 302
                lm_update(model, optimizer, [longest] * candidate, Progress(), loss_id=loss_id, microbatch=candidate)
                model.load_state_dict(initial)
                optimizer = lm_optimizer(model)
                training = [
                    lm_update(model, optimizer, sequences[offset : offset + 64], state, loss_id=loss_id, microbatch=candidate)
                    for offset in (0, 64)
                ]
        ...
        if microbatch is None:
            raise RuntimeError("No frozen microbatch candidate fits this GPU")
        assert state.update == 2 and state.next_data_cursor == 128
        checks["selected_microbatch"] = microbatch
        checks["maximum_length_forward_backward"] = 302
        checks["training"] = training
        checks["resume"] = check_resume(
            model, optimizer, state, sequences, out, cell, loss_id, inputs, device, architecture, microbatch
        )
        checks["interpretation"] = check_interpretation(model, records, microbatch)
        general = evaluate_records(model, general_slice, microbatch=microbatch)
        pairs = evaluate_first_repeat(model, pairs_fixture, microbatch=microbatch)
    ...
    reference = without_rows(pairs)
    debug_general = dict(answer_ce=general["answer_ce"], accuracy=general["accuracy"])
    gate_debug = gate(
        debug_general,
        {
            name: dict(accuracy=debug_general["accuracy"])
    ...
    result = dict(
        ...
        environment=info,
        cells=cells,
        selection_code_path=dict(candidate=CELLS[0], primary_metric="first_member_42_cell_macro_answer_ce", debug_only=True),

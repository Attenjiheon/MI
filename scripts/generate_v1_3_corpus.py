"""Build the immutable v1.3 reserved splits and 32M ordered train stream."""
...
from corpus.generate import Builder, Stats, dump, file_hash, self_test, write_json
from corpus.language import OPS, PATTERNS, digest
from corpus.replay import replay, validate
from corpus.v1_3 import cells, rng as v13_rng, sample_pair, seed as v13_seed
...
def load_prior_registry(base: Path) -> tuple[set[str], set[str]]:
    ...
    for path in sorted(base.rglob("*.jsonl.gz")):
        if "provenance" in path.parts or ("causal_pairs" in path.parts and not path.name.endswith(".origins.jsonl.gz")):
            continue
...
class V13Builder(Builder):
    ...
    def first_repeat(self, split: str, quota: int):
        ...
        with gzip.open(path, "wt", encoding="utf-8", compresslevel=3) as handle:
            for cell in cells():
                generator = v13_rng(f"{split}_first_repeat", cell.index)
                accepted = attempts = 0
                rejects = Counter()
                ...
                summary[cell.cell_id] = {
                    ...
                    "rng_seed": v13_seed(f"{split}_first_repeat", cell.index),
...
def run():
    ...
    out = ROOT / design["data"]["root"]
    if out.exists():
        raise FileExistsError("Preserve an existing v1.3 corpus; never overwrite it")
    if file_hash(base / "manifest.json") != json.loads((ROOT / "experiment_v1_3/design_manifest.json").read_text())["v1_2_evidence_inputs"]["data/language_v1_2/manifest.json"]:
        raise ValueError("v1.2 base manifest differs from frozen design input")
    ...
    print("loading all v1.0-v1.2 exact-sequence and READ-prefix hashes", flush=True)
    ...
    provenance = out / "provenance/v1_2"
    ...
    builder = V13Builder(out, prior_hashes.copy(), prior_prefixes.copy())
    ...
    # Copied prefix statistics are loaded from the v1.2 manifest for coverage.
    base_stats = json.loads((base / "corpus_statistics.json").read_text())
    prior_train_stats = [value for key, value in base_stats.items() if key.startswith("train_shards/")]
    ...
    validation = {
        **self_test(),
        "status": "passed",
        "schema": "language-v1.3-cpu-validation",
        ...
        "v1_2_train_prefix_byte_identical": True,
    ...
    manifest = {
        "schema_version": "language-v1.3",
        "generator_version": "1.3.0",
        "experiment_version": "v1.3",
        "profile": "full",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "completed_at": datetime.now(timezone.utc).isoformat(),
        "design_config_sha256": file_hash(design_path),
        "base_manifest_sha256": file_hash(base / "manifest.json"),
        "seed_derivation": "SHA256(20260917|language-v1.3|<purpose>|<index>)[:8], unsigned big endian",
    ...
    # v1.3 performs a versioned audit below instead of weakening that auditor.
    audit_v1_3(out)
    results = ROOT / "experiment_v1_3/results"
...
def audit_v1_3(root: Path):
    ...
    pair_counts = {}
    pair_hashes = set()
    for split, quota in (("select", 16), ("gate", 64), ("test", 128)):
        counts = Counter()
        members = set()
        answers = set()
    ...
    result = {
        "status": "passed",
        "schema": "language-v1.3-postwrite-audit",
        ...
        "v1_2_train_prefix_byte_identical": "passed",

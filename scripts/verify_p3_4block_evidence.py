...
from interp_v1_1.runtime import sha,verify_inputs,deterministic
from interp_v1_1.data import token_rows
from interp_v1_1.cli import evaluate_gate_suite,metric_summary
from interp_v1_1.model import Transformer

base=ROOT/'experiment_v1_1/evidence/p3_colab_20260916T085123549826'
...
for name in ('init.pt','best.pt','last.pt'):
    ...
    assert sum(v.numel() for v in p['model'].values())==797184
...
report=dict(status='passed',meaning='Evidence consistency verified; behavior gate FAILED',decision='failed',
    evidence_zip_sha256=sha(base.parent/'p3_4block_evidence_20260916T085123549826.zip'),
...
out=ROOT/'experiment_v1_1/results/p3_evidence_verification.json';out.write_text(json.dumps(report,indent=2,allow_nan=False)+'\n')

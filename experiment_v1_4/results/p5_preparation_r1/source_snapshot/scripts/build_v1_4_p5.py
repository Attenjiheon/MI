...
FOLDER=ROOT/'experiment_v1_4/p5_r1'
PREP=ROOT/'experiment_v1_4/results/p5_preparation_r1'
...
def prepare():
    ...
    config=dict(schema='p5-read-v1.4-r1',data_root=data_root,models=models,init_sources=sources,
...
print('입력 검증 완료')''')
    ...
    code("subprocess.run([sys.executable,'-m','pip','install','-r','experiment_v1_4/frozen_test_r1/requirements-primary.lock.txt'],check=True)\nsubprocess.run([sys.executable,'-m','pytest','tests_v1_4/test_p5.py','-q'],check=True)")
    md('## 영속 저장·재개\nDrive의 전용 폴더를 사용합니다. 저장 파일을 수정하지 마세요. 기존 결과와 다른 계약은 자동으로 거부합니다. 새 런타임에서는 Drive의 완료 조각을 로컬로 복사하고 checksum을 검사합니다.')
    code('''from google.colab import drive
drive.mount('/content/drive')
PERSIST=Path('/content/drive/MyDrive/boolean_interp_v1_4/P5_r1')
OUT=ROOT/'experiment_v1_4/runs/p5_r1'
...
files.download(str(archive))''')
    return dict(nbformat=4,nbformat_minor=5,metadata=dict(kernelspec=dict(display_name='Python 3',language='python',name='python3'),colab=dict(name='P5_v1_4_READ_r1.ipynb'),accelerator='GPU'),cells=cells)
...
def pack():
    ...
    bundle=ROOT/'experiment_v1_4/bundles/v1_4_p5_bundle_r1.zip'
    ...
    dest=ROOT/'experiment_v1_4/notebooks/P5_v1_4_READ_r1.ipynb';write(dest,nb)

"""v1.3 LM runs: one frozen pilot cell or the promoted winner, milestone-only evaluation.

Pilot cells consume the first 8M prediction tokens of the frozen stream; the confirmatory
stage continues the same stream to 32M. Selection uses `select/` only. `gate/` is read once,
after training, at the single selected checkpoint. `test/` is never opened here.
"""
...
EFFECTIVE_BATCH = 64
CODE_FOLDERS = ("interp_v1_3", "interp_v1_2", "corpus")


def split_cell(cell):
    architecture, loss = cell.rsplit("_", 1)
    if architecture not in ARCHITECTURES or loss not in ("uniform", "read4"):
        raise ValueError("Unknown pilot cell: " + cell)
...
def stage_contract(root, stage, cell):
    ...
    config = json.loads((root / "experiment_v1_3/configs/run.json").read_text())
    transformer = json.loads((root / "experiment_v1_3/configs/transformer.json").read_text())
    if cell not in config["pilot_cells"]:
        raise ValueError("Cell is not one of the six frozen pilot cells")
    if stage == "pilot":
        budget = config["pilot_nominal_prediction_tokens"]
        milestones = list(config["pilot_milestones"])
    elif stage == "confirm":
        budget = transformer["training"]["confirmatory_nominal_prediction_tokens"]
        milestones = list(transformer["checkpoint_selection"]["candidate_milestones_prediction_tokens"])
    else:
    ...
    sizes = {entry["id"]: entry["expected_trainable_parameters"] for entry in transformer["architectures"]}
    return dict(
        config=config,
        ...
        loss=loss,
        expected_parameters=sizes[architecture],
...
def evaluate_select(model, data_root, microbatch, *, debug_records=None):
    ...
    if pairs["pair_count"] != 672 or len(pairs["first"]["cell"]["cells"]) != 42:
...
def evaluate_gate_suite(model, data_root, thresholds, microbatch):
    ...
    if pairs["pair_count"] != 2688 or len(pairs["first"]["cell"]["cells"]) != 42:
...
def select_checkpoint(milestones, stage, budget, near_tie):
    """Pilot compares the final milestone only; confirm minimizes first-member macro CE."""
    if stage == "pilot":
...
def run(a):
    ...
    hashes["run_settings"] = dict(
        schema="lm-run-v1.3", stage=a.stage, cell=a.cell, lm_seed=a.lm_seed, debug=a.debug
    )
    if not a.debug:
        if a.device != "cuda" or not torch.cuda.is_available():
            raise ValueError("Production v1.3 runs require a CUDA runtime")
        smoke = json.loads(a.smoke_report.read_text()) if a.smoke_report else {}
        if smoke.get("status") != "passed" or smoke.get("scope") != "cuda":
            raise ValueError("A current v1.3 GPU smoke report is required")
        ...
        code = {k: v for k, v in hashes["code"].items() if k.startswith("interp_v1_3/")}
        if smoke["input_hashes"]["code"] != code:
    ...
    if a.resume:
        ...
        if len(rows) != frozen["final_cursor"] or sum(len(r) - 1 for r in rows) != frozen["actual_prediction_tokens"]:
            raise ValueError("Train stream differs from the frozen 32M contract")
    ...
    while state.prediction_tokens < budget and (not a.debug or state.update < debug_end):
        ...
        model_before = cpu_copy(model.state_dict())
        optimizer_before = cpu_copy(optimizer.state_dict())
        while True:
            try:
                if a.device == "cuda":
    ...
    selected = select_checkpoint(milestones, "pilot" if a.debug else a.stage, budget, contract["near_tie_nats"])
    selected["sha256"] = sha(output / selected["checkpoint"])
    gate_result = None
    if a.stage == "confirm" and not a.debug:
        graded = Transformer(contract["architecture"]).to(a.device)
        graded.load_state_dict(torch.load(output / selected["checkpoint"], map_location=a.device, weights_only=False)["model"])
        gate_result = evaluate_gate_suite(
            graded, data_root, json.loads((root / "experiment_v1_3/configs/evaluation.json").read_text())["gate"], microbatch
        )
    ...
    manifest = dict(
        ...
        status="debug_only"
        if a.debug
        else ("completed" if a.stage == "pilot" else ("passed" if gate_result["decision"]["passed"] else "failed")),
        debug_only=a.debug,
        hashes=hashes,
        environment=env,

...
def run(root,out,device):
    ...
    records=json.loads((root/'experiment_v1/debug/sequences.json').read_text())
    sequences=[e['token_ids'] for e in records]
    inputs['debug']=sha(root/'experiment_v1/debug/sequences.json')
    inputs['code']={str(p.relative_to(root)):sha(p) for p in sorted((root/'interp').glob('*.py'))}
    checks={}; model=Transformer().to(device)
    assert sum(p.numel() for p in model.parameters())==400640
    assert model.embedding.weight.data_ptr()!=model.unembedding.weight.data_ptr()
    checks['lm_parameters']=400640

# Codex 프로젝트 지침 — 실험 세션 진입 규칙

이 파일은 프로젝트 세션을 시작할 때 가장 먼저 읽는 **라우터**다. 세부 실험값을 요약해 두는 문서가 아니며, 실제 규격은 아래 원문에서 확인한다.

## 0. Codex 작업 방식

- 이 저장소의 Codex 공통 지침은 루트 `AGENTS.md`에서 관리한다. 하위 작업 경로에 추가 `AGENTS.md`가 있으면 해당 범위의 지침도 확인한다. `archive/`의 과거 지침은 당시 실험을 감사하기 위한 보존 원문이다.
- 세션 시작 시 작업 디렉터리, 현재 Git 브랜치, `git status --short`를 확인한다. 기존 사용자 변경과 다른 작업의 미커밋 파일을 보존하고, 이번 요청에 해당하는 파일만 수정·stage·commit한다.
- 파일·본문 검색에는 `rg --files`와 `rg`를 우선 사용한다. 실행 도구와 사용 가능한 권한은 현재 Codex 세션을 기준으로 확인하고, 다른 에이전트의 전용 명령·자동 메모리·설치 상태를 전제하지 않는다.
- 승인된 범위의 조사·구현·검증은 끝까지 진행한다. 샌드박스나 네트워크 제한으로 필요한 명령이 막히면 Codex의 권한 요청 절차를 사용하며, 실행하지 못한 명령을 성공으로 보고하지 않는다.
- 작업 전 목표와 확인할 증빙을 한국어로 짧게 알리고, 진행 중에는 주요 발견과 장애를 공유한다. 완료 보고에는 변경 파일, 실제 검증 결과, 남은 작업, Git commit과 push 결과를 포함한다.
- 코드 변경은 관련 `tests_v1_4/` 검증과 해당 단계의 필수 확인을 수행한다. 문서만 바꾸는 작업은 링크·diff·보존 범위를 확인하며, 이를 이유로 학습이나 동결 gate/test를 재실행하지 않는다.
- 긴 작업을 재개하거나 대화가 압축된 뒤에는 `experiment_v1_4/CURRENT.md`, 해당 단계 상태 문서와 실제 증빙에서 진행 상태를 복원한다. 이 파일의 날짜가 붙은 요약이나 대화 기억만으로 완료를 판단하지 않는다.

Codex 지침 파일의 적용 방식은 [공식 AGENTS.md 안내](https://learn.chatgpt.com/docs/agent-configuration/agents-md)를 참고한다. 실험 규격의 우선순위와 gate는 아래 규칙을 따른다.

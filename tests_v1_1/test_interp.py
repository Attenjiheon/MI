...
import pytest
import torch
from interp_v1_1.model import Transformer,batch
from interp_v1_1.data import extract
from interp_v1_1.dictionary import Dictionary,statistics
from interp_v1_1.patching import direction_patch,coordinate_patch,patch
from interp_v1_1.training import Progress,lm_optimizer,lm_update
from interp_v1_1.runtime import deterministic,save,restore
from interp_v1_1.probe import fit_probe,select_prefix
from interp_v1_1.behavior import GATE_DIAGNOSTICS,adjudicate,may_extend
from interp_v1_1 import cli
...
def test_cache_exact_join_read_and_update():
    records=json.loads(Path('experiment_v1_1/debug/sequences.json').read_text())[:2]
    model=Transformer()
...
def test_p3_snapshot_recovery_ignores_partial_copy(tmp_path):
    from interp_v1_1.persistence import publish, recover
...
def test_four_block_architecture_and_gradients():
    model=Transformer()
    assert len(model.blocks)==4
    assert sum(p.numel() for p in model.parameters())==797184
    ids,mask,targets=batch([[1,3,9,13,8,9,13,2]])
    from interp_v1_1.model import ce_sum
    ce_sum(model(ids,mask),targets).backward()
    for block in model.blocks:
        assert block.qkv.weight.grad.abs().sum()>0
        assert block.mlp_out.weight.grad.abs().sum()>0
    two_block_state={k:v for k,v in model.state_dict().items()
                     if not k.startswith(('blocks.2.','blocks.3.'))}
    with pytest.raises(RuntimeError,match='Missing key'):
        model.load_state_dict(two_block_state)


def test_four_block_depth_scaled_initialization(monkeypatch):
    def constant_init(tensor,**kwargs):
        with torch.no_grad(): tensor.fill_(.02)
        return tensor
    monkeypatch.setattr(torch.nn.init,'normal_',constant_init)
    model=Transformer()
    for block in model.blocks:
        torch.testing.assert_close(block.attn_out.weight,torch.full_like(block.attn_out.weight,.02/(8**.5)))
        torch.testing.assert_close(block.mlp_out.weight,torch.full_like(block.mlp_out.weight,.02/(8**.5)))
        torch.testing.assert_close(block.qkv.weight,torch.full_like(block.qkv.weight,.02))

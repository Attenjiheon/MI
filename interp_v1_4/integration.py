...
def check_interpretation(model, records, microbatch):
    ...
    with torch.no_grad():
        for offset in range(0, len(records), microbatch):
            ...
            for layer, block in enumerate(model.blocks):
                mid = cached[f"blocks.{layer}.resid_mid"]
                u = cached[f"blocks.{layer}.mlp_in"]
                m = cached[f"blocks.{layer}.mlp_out"]
                h = cached[f"blocks.{layer}.resid_post"]
            ...
            for row, record in enumerate(chunk):
                for event in record["read_events"]:
                    ...
                    for name, hook in (("h", "resid_post"), ("u", "mlp_in"), ("m", "mlp_out")):
                        pools[name].append(cached[f"blocks.0.{hook}"][row, pos].cpu())
    ...
    for kind, hook, source_name, target_name in (
        ...
        original = cached[f"blocks.0.{hook}"][:, pos]
        with torch.no_grad(), patch(model, f"blocks.0.{hook}", [pos], original):
            torch.testing.assert_close(original_logits, model(ids, mask), atol=0, rtol=0)
        donor = pools[target_name][1:2]
        with torch.no_grad(), patch(model, f"blocks.0.{hook}", [pos], donor):
        ...
        with torch.no_grad():
            ...
            with patch(model, f"blocks.0.{hook}", [pos], value):
                assert torch.isfinite(model(ids, mask)).all()

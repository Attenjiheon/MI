...
def run(root,out,device):
    ...
    records=json.loads((root/'experiment_v1_2/debug/sequences.json').read_text())
    sequences=[e['token_ids'] for e in records]
    inputs['debug']=sha(root/'experiment_v1_2/debug/sequences.json')
    inputs['code']={str(p.relative_to(root)):sha(p) for p in sorted((root/'interp_v1_2').glob('*.py'))}

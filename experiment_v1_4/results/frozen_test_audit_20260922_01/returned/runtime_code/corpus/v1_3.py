"""v1.3 seed namespace and grammar-preserving first/repeat pair construction."""
...
SEED_NAMESPACE = "20260917|language-v1.3"
DEPTH_BINS = ("1", "2-3", "4+")
...
def make_pair(initial: list[int], program: list, split: str, rng_seed: int, cell: Cell, chooser: np.random.Generator) -> dict | None:
    ...
    return {
        "schema_version": "language-v1.3-first-repeat-pair",
        "split": split,
...
def sample_pair(generator: np.random.Generator, split: str, cell: Cell, max_attempts: int = 100_000) -> tuple[dict, int, dict]:
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if not pattern_ok(program):
            ...
            continue
        pair = make_pair(initial, program, split, seed(split, cell.index), cell, generator)
        if pair is None:
